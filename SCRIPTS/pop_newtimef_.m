...
function varargout = pop_newtimef_(Traj,width,start_xpos, EEG, typeproc, num, tlimits, cycles, varargin );
...
if nargin < 4
      ...
      varargout{1} = sprintf('figure; pop_newtimef( %s, %d, %d, [%s], [%s] %s);', inputname(1), typeproc, num,int2str(tlimits), num2str(cycles), options);
...
hold on
plot(Traj,'m','LineWidth',1)
pos = get(gca, 'Position');

pos(1) = pos(1)+start_xpos;
